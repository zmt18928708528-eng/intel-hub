import { getJson, getText, nowIso } from "../http.ts";
...
const OFFICIAL_MSRP: Array<{ title: string; price: number; note: string }> = [
  { title: "Mac mini M4 16GB/256GB (new MSRP ref)", price: 799, note: "US list seen on Apple refurb compare; confirm on buy page." },
  { title: "Mac mini M4 Pro 24GB/512GB (new MSRP ref)", price: 1379, note: "Typical US starting config; confirm on buy page." },
];
...
interface ProductLd {
  ...
  description?: string;
  url?: string;
  offers?: { price?: string | number; priceCurrency?: string; sku?: string } | Array<{
    price?: string | number;
